...
- All **exported entities** are prefixed with `smp` using correct casing:
    - Classes: `SmpSomeClass`
    - Constants/functions: `smpSomeConst`
- **Enums** must be **plural**, e.g. `SmpValueTypes`

## 🟦 Documentation guidelines
* We use Markdown files to document complex features
* All documents are written in British English

## Alignment From `@kikocosmeticsorg/uc-api-nest-common-fe`

When the task is aligning this project to `@kikocosmeticsorg/uc-api-nest-common-fe`, follow these rules strictly.

### Import Mapping Rules

- Exception: `KikLoggerService` doesn't need to be reexported from here, so it shouldn't appear in this library.
- Ignore: stuff related to the user (like user zod schemas), marketing cloud, error messages

### Renaming Rules

- Rename entities “smartly”: update class names, function names, variable names, and exported identifiers.
- Also update string values if they contain `kik` (e.g. metadata keys, labels, constants), preserving casing and meaning.
